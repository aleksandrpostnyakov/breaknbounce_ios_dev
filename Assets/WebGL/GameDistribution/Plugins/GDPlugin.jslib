  ...
  SDK_Init: function(gameKey) {
    ...
    window["GD_OPTIONS"] = {
      ...
      onEvent: function(event) {
        switch (event.name) {
          case "SDK_GAME_START":
            SendMessage("WebGLProvider", "GDResumeGame");
            break;
          case "SDK_GAME_PAUSE":
            SendMessage("WebGLProvider", "GDPauseGame");
            break;
          case "SDK_REWARDED_WATCH_COMPLETE":
            SendMessage("WebGLProvider", "GDRewardGame");
            break;
          case "SDK_ERROR":
            break;
        }
    ...
    if (
      ...
      gdsdk.preloadAd(gdsdk.AdType.Rewarded)
        .then(function(response){
          SendMessage("WebGLProvider", "GDPreloadRewardedVideo",1);
        })
        .catch(function(error){
          SendMessage("WebGLProvider", "GDPreloadRewardedVideo",0);
        });
    }
  },
    ...
    if (typeof gdsdk !== "undefined" && typeof gdsdk.showAd !== "undefined") {
      ...
      .then(function(response){
        if(adType===gdsdk.AdType.Rewarded){
          SendMessage("WebGLProvider", "GDRewardedVideoSuccess");
        }
      })
      .catch(function(error){
        if(adType===gdsdk.AdType.Rewarded){
          SendMessage("WebGLProvider", "GDRewardedVideoFailure");
        }
      });
    }
  }
